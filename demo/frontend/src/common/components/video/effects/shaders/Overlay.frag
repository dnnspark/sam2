...
void main() {
  ...
  vec2 adjustedClickCoord =
      calculateAdjustedTexCoord(vTexCoord, vec4(uClickPos, uClickPos + 0.1), aspectRatio);
  ...
  for (int i = 0; i < MAX_MASKS; ++i) {
    ...
    float maskValue = texture(uMaskTexture[i], tvTexCoord).r;
    ...
    if (i == uActiveMask && uTime < timeThreshold) {
    ...
    if (edgeColor.a <= 0.0f) {
      edgeColor = detectEdges(uMaskTexture[i], 1.25, maskColor);
    }
  }
