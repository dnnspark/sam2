    ...
    with av.open(path) as cont:
        ...
        if num_video_streams > 0:
            ...
            if side_data is not None:
                rotation_deg = side_data.get("DISPLAYMATRIX", 0)
            else:
                rotate_meta = video_stream.metadata.get("rotate") if hasattr(video_stream, "metadata") else None
                if rotate_meta is not None:
                    try:
                        rotation_deg = float(rotate_meta)
